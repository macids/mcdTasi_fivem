...
RegisterNetEvent('mcdTasi:nkoTasi')
AddEventHandler('mcdTasi:nkoTasi', function()
    TriggerEvent("mcdTasi:kontrol", "kucak")
end)

RegisterNetEvent('mcdTasi:mcdRehinAl')
AddEventHandler('mcdTasi:mcdRehinAl', function()
    TriggerEvent("mcdTasi:kontrol", "rehin")
end)
...
local bussy = true
RegisterNetEvent('mcdTasi:kontrol')
AddEventHandler('mcdTasi:kontrol', function(xtip)
    if bussy then
        bussy = false
        local playerPed = PlayerPedId()
        if not IsEntityAttached(playerPed) then
            QBCore.Functions.Notify('İşlem Gerçekleştiriliyor!', "primary", 2300)
            if not aktif then Wait(2500) end
            if targetId == 0 then
            ...
            if not aktif then 
                ...
                if tip == "kucak" then
                    ...
                    QBCore.Functions.Notify('[X] ile kucakladığın oyuncuyu bırakabilirsin', "primary", 4000)
                ...
                TriggerServerEvent("mcdTasi:server:oyuncu-2-anim", targetId, tip)
                aktif = true
            else
                QBCore.Functions.Notify('Oyuncu Bırakılıyor!')
                if tip == "rehin" then
                    TriggerServerEvent("mcdTasi:server:rehin-birak", targetId, "birak")
                    rehineBirak("oyuncu1")
                else
                    TriggerServerEvent("mcdTasi:server:anim-iptal", targetId)
                    animIptal()
                end
            end
        end
...
RegisterNetEvent('mcdTasi:client:anim-iptal')
AddEventHandler('mcdTasi:client:anim-iptal', function()
    animIptal()
end)
...
RegisterNetEvent('mcdTasi:client:oyuncu-2-anim')
AddEventHandler('mcdTasi:client:oyuncu-2-anim', function(xtip, oyuncu1)
    if not aktif then
        local playerPed = PlayerPedId()
        ...
        if aktif then
            ...
            if oyuncu == "oyuncu1" and tip == "rehin" then
                ...
                if IsDisabledControlJustPressed(0,74) then --kill 
                    ...
                    if ammo > 0 then
                        TriggerServerEvent("mcdTasi:server:rehin-birak", targetId, "oldur")
                        rehineOldur("oyuncu1")
                    else
...
RegisterNetEvent('mcdTasi:client:rehin-birak')
AddEventHandler('mcdTasi:client:rehin-birak', function(rehintip)
